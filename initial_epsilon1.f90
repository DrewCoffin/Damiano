...
!                y(k+n21)=-Amp*f(theta(1,j))/fmax*
                y(k+n21)=-Amp*f_e1(theta(1,j))/fmax* &
!     &                exp(-((pi/2.-theta(n2/2,j)-0.0d0))**2/d1**2)*
  ...
  55      format(5e13.5)
          ...
          double precision :: nu11,nu13,f_e1
...
!     &      sin(3.d0*pi*s/so))
           f_e1=cos(lambda)**3*(1.09828*sin(1.89472*s) + 3.87981*10**(-11)* &
     &                    sin(3.78945*s) - 0.061285*sin(5.68417*s))
          return
          END FUNCTION f_e1
